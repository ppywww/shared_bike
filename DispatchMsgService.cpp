...
void DispatchMsgService::subscribe(u32 eid, iEventHandler* handler)//根据事件id找到对应的事件处理器，把对应事件id的handler添加到事件订阅者列表当中，事件订阅者列表保存事件id和事件处理器的映射关系
{
	//LOG_DEBUG("DispatchMsgService::subscribe eid:%d\n", eid);
	T_EventHandlersMap::iterator iter = subscribers_.find(eid);//先根据事件id找到对应的事件处理器
	...
	if (iter != subscribers_.end())//如果找到了对应事件id的事件处理器，那么就把对应事件id的handler添加到事件订阅者列表当中
	{
		
		T_EventHandlers::iterator hdl_iter = std::find(iter->second.begin(), iter->second.end(), handler);
		if (hdl_iter == iter->second.end())//如果事件处理器列表当中没有找到对应事件id的handler，那么就把handler添加到事件处理器列表当中
		{
			iter->second.push_back(handler);

		}
		//不然就说明事件处理器列表当中已经有了对应事件id的handler，那么就不需要再添加了，避免重复添加同一个事件处理器
	}
	else//如果没有找到对应事件id的事件处理器，那么就新建一个事件处理器列表，把handler添加到事件处理器列表当中，然后把事件id和事件处理器列表的映射关系添加到事件订阅者列表当中
	{
...
void DispatchMsgService::svc(void* argv)//线程池中线程的回调函数，处理事件
{
	DispatchMsgService* dms = DispatchMsgService::getInstance();
	
	iEvent* ev = (iEvent*)argv;
	if (!dms->svr_exit_)//如果服务器没有退出，那么就处理事件
	{
...
//派发处理事件
iEvent* DispatchMsgService::process(const iEvent* ev)//根据事件id找到对应的事件处理器进行处理
{
	...
	T_EventHandlersMap::iterator handlers = subscribers_.find(eid);//根据事件id找到对应的事件处理器
	...
	while (!done)
	{
		...
		{			
			...
			switch (EventId)
			{
				...
				//广播响应
				case EEVENTID_BROADCAST_MSG_RSP://这个响应事件是由服务器主动生成的，所有客户端都要响应，所以在这里直接广播到各个客户端
				{
...
void DispatchMsgService::sendPesponseMessage(iEvent* ev, EventID Eid, NetworkInterface* interface)//把序列化后的响应消息发送给客户端
{
	ConnectSession* cs = (ConnectSession*)ev->get_args();//拿到连接会话
	cs->response = ev;//把响应事件保存到连接会话当中
	//系列化请求数据
	...
	*(i32*)(cs->write_buf + 6) = cs->message_len;
	...
	interface->send_response_message(cs);//把响应消息发送给客户端
}
