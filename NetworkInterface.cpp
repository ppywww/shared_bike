...
bool NetworkInterface::start(int port)
{
	...
	base_ = event_base_new();//创建一个新的事件循环对象 用来管理和分发事件
...
void NetworkInterface::handle_response(bufferevent* bev, void* arg)//回复请求回调函数，暂时没有用到，后续可以在这里处理一些回复请求的事件，比如说发送响应消息等
{
...
{
	event_base_loop(base_, EVLOOP_NONBLOCK);//非阻塞方式运行事件循环，处理网络事件
	//处理响应事件，回复响应消息 --- 未完待续
