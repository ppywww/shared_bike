...
{
	mysql_ = mysql_init(mysql_);//传入一个MYSQL结构体的指针，
	//mysql_init函数会初始化这个结构体，并且返回这个结构体的指针，如果传入的指针为NULL，mysql_init函数会自己分配一个MYSQL结构体，并且返回这个结构体的指针
	if (mysql_ == NULL)
	{
		LOG_ERROR("init mysql failed %s, %d", this->GetErrInfo(), errno);
		return false;
	}
	//// MySQL 8.0+ 中自动重连已是默认行为，不再需要设置 MYSQL_OPT_RECONNECT选项。
	/*char cAuto = 1;
	...
	if (mysql_real_connect(mysql_, szHost, szUser, szPasswd, szDb, nPort, NULL, 0) == NULL)//连接数据库，连接成功返回MYSQL结构体的指针，连接失败返回NULL
	{
