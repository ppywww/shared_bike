...
protected:
	...
	static std::forward_list<struct bufferevent*> m_userList;//用户列表，保存当前连接的用户 
	//forward_list是C++11新增的单链表存储相同个数的同类型元素，单链表耗用的内存空间更少，空间利用率更高，并且对于实现某些操作单链表的执行效率也更高。
	
	typedef std::vector<iEventHandler*> T_EventHandlers;//用户事件管理容器
	typedef std::map<u32, T_EventHandlers> T_EventHandlersMap;//事件id和事件处理器的映射关系容器
	T_EventHandlersMap subscribers_;//事件订阅者列表，保存事件id和事件处理器的映射关系
